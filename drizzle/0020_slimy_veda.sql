...
CREATE TABLE `__new_song` (
	...
	FOREIGN KEY (`current_version_id`) REFERENCES `song_version`(`id`) ON UPDATE no action ON DELETE no action,
	FOREIGN KEY (`current_illustration_id`) REFERENCES `song_illustration`(`id`) ON UPDATE no action ON DELETE no action
);
