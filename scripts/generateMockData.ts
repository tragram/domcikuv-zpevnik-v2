...
import { song, songChange } from "../src/lib/db/schema/song.schema";
...
function generateChordproURL(): string {
  ...
  const filename = `${faker.string.alphanumeric({ length: 8 })}_${Date.now()}.chordpro`;
...
function generateMockSongChanges(
  ...
  const songChanges: MockSongChange[] = [];
  ...
  for (const songId of songIds) {
    ...
    for (let i = 0; i < numChanges; i++) {
      ...
      const timestamp = faker.date.between({ 
        from: new Date(songStartDate.getTime() + (i * 24 * 60 * 60 * 1000)), 
        to: new Date()
      });
      ...
      songChanges.push({
        ...
        // Trusted users and admins get auto-verified changes, others have 80% chance
        verified: user.isTrusted || user.isAdmin || faker.datatype.boolean({ probability: 0.8 }),
      });
    }
  }
  
  // Sort by timestamp for realistic chronological order
  return songChanges.sort((a, b) => a.timestamp.getTime() - b.timestamp.getTime());
}

async function insertMockUsers(db: DrizzleD1Database, users: MockUser[]): Promise<number> {
  let successCount = 0;
  let errorCount = 0;
...
async function insertMockSongChanges(db: DrizzleD1Database, songChanges: MockSongChange[]): Promise<number> {
  let successCount = 0;
  let errorCount = 0;
  
  console.log('Inserting mock song changes into database...');
  
  for (const changeData of songChanges) {
    try {
      await db
        .insert(songChange)
        .values({
  ...
  console.log(`✅ Song changes: ${successCount} successful, ${errorCount} errors`);
  return successCount;
}
...
async function fetchExistingSongIds(db: DrizzleD1Database): Promise<string[]> {
  try {
    ...
    throw new Error('Could not fetch existing songs. Make sure the song table is populated first.');
  }
}
...
async function generateMockData(
  ...
  if (songIds.length === 0) {
    throw new Error('No songs found in database. Please populate the song table first.');
  }
  ...
  console.log(`Generated ${users.length} users and ${songChanges.length} song changes`);
  ...
  console.log(`- Song changes inserted: ${changeInserts}/${songChanges.length}`);
  ...
  console.log(`- Verified changes: ${verifiedChanges}/${songChanges.length} (${Math.round(verifiedChanges/songChanges.length*100)}%)`);
}
...
async function main(): Promise<void> {
  try {
    ...
    const userCountArg = process.argv.find(arg => arg.startsWith('--users='))?.split('=')[1];
    const minChangesArg = process.argv.find(arg => arg.startsWith('--min-changes='))?.split('=')[1];
    const maxChangesArg = process.argv.find(arg => arg.startsWith('--max-changes='))?.split('=')[1];
    ...
    console.log(`- Changes per song: ${changesPerSong.min}-${changesPerSong.max}`);
    console.log('');
