...
# Load secrets using a fallback mechanism for CI environments
def get_secrets():
    try:
        # First try the normal load_secrets function
        return load_secrets()
    except Exception as e:
        print(f"Warning: Could not load secrets using normal method: {e}")
        # Fall back to reading from environment or simple secrets file
        if os.path.exists("secrets.yaml"):
            with open("secrets.yaml", "r") as f:
                return yaml.safe_load(f)
        else:
            # Create a minimal secrets dict from environment variables
            return {
...
def retrieve_model_prompt(
    prompt_path: Path, model: str | None, prompt_id: str | None
) -> str | None:
    # settings model or prompt_id to None will not match that part
    # TODO this does not work with different models and GENERATE ONLY MISSING
    if not prompt_path.exists():
        return None
    with open(prompt_path, "r", encoding="utf-8") as f:
        data = yaml.safe_load(f)
    for d in data:
        if (model is None or model in d["model"]) and (
            prompt_id is None or d["prompt_id"] == prompt_id
        ):
            return d["response"]
    return None
...
def generate_missing_prompts(prompt_model, prompt_id):
    ...
    for i, song in enumerate(songs):
        existing_prompt = retrieve_model_prompt(
            prompt_path(song.stem),
            prompt_model,
            None if GENERATE_ONLY_MISSING else prompt_id,
        )
...
def generate_missing_images(
    ...
    for song_prompt in Path(songs_path() / "image_prompts").glob("*.yaml"):
        ...
        image_filename = image_folder / model2filename(
            prompt_model, prompt_id, image_model
        )
        if image_filename.is_file() or (
            GENERATE_ONLY_MISSING and len(os.listdir(image_folder)) > 0
        ):
            print(
                f"{song_prompt.stem} already generated with {image_model} --> skipping"
            )
        else:
            print(
                f"Saving illustration for {song_prompt.stem} generated by {image_model}"
            )
...
if __name__ == "__main__":
    generate_missing_prompts(
        prompt_id=SELECTED_PROMPT, prompt_model=SELECTED_PROMPT_MODEL
    )
    generate_missing_images(
        prompt_model=SELECTED_PROMPT_MODEL,
