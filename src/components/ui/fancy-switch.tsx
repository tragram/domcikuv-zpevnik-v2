...
export default function FancySwitch({ options, selectedOption, setSelectedOption, roundedClass, children, vertical = false, full = false }) {
    return (
        <div className={"h-full rounded-full transition-all shadow-sm flex outline-white dark:outline-primary outline-2 outline bg-backround " + (full ? "w-full justify-center" : "w-fit")}>
            <FancySwitchBase
                options={options}
