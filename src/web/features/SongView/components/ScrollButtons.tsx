...
const ScrollButtons = memo(({ fitScreenMode }: ScrollButtonsProps) => {
  ...
  useEffect(() => {
    const resizeObserver = new ResizeObserver(() => {
      setShowScrollButtons(
        document.body.scrollHeight - MAGICAL_FIREFOX_CONSTANT_PX >
          window.outerHeight && fitScreenMode !== "fitXY",
      );
    });
  ...
  const scrollDown = () => {
    const remainingContent =
      document.body.scrollHeight - window.scrollY - window.outerHeight;
    ...
    for (const container of sections) {
      ...
      if (rect.bottom >= window.outerHeight) {
        ...
        scroll.scrollTo(rect.top + window.scrollY - offset, {
    ...
    scroll.scrollTo(window.scrollY + scrollDist, {
      duration: scrollDist / scrollSpeed,
    });
  };
